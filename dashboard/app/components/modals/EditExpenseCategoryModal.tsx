...
export interface EditExpenseCategoryModalProps {
  open: boolean;
  onClose: () => void;
  category: ExpenseCategory | null;
  onUpdate: (id: string, data: { name: string; slug: string; description?: string; icon: string }) => Promise<any>;
  onSuccess: (updatedCategory: ExpenseCategory) => void;
}
...
export default function EditExpenseCategoryModal({
  ...
  const onSubmit = async (data: any) => {
    if (!category) return;
    try {
      const res = await onUpdate(category.id, data);
      onSuccess(res.data);
      reset();
    } catch (err: any) {
      setError("root", {
