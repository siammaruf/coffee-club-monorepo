...
export class ReportService implements OnModuleInit {
    ...
    private readonly REPORT_CRON_NAME = 'auto-daily-report';
    ...
    async onModuleInit() {
        try {
            const config = await this.whatsappConfigRepository.findOne({ where: {} });
            if (config?.auto_report_generation_enabled) {
                this.registerReportCron(config.auto_report_generation_time || '00:00');
            } else {
                // Default: register at midnight if no config exists
                this.registerReportCron('00:00');
            }
        } catch {
            this.logger.warn('No WhatsApp config found, registering default midnight cron');
            this.registerReportCron('00:00');
        }
    }

    async updateReportSchedule(time: string, enabled: boolean) {
        try {
            if (this.schedulerRegistry.doesExist('cron', this.REPORT_CRON_NAME)) {
                this.schedulerRegistry.deleteCronJob(this.REPORT_CRON_NAME);
            }
        } catch {
            // Job does not exist
        }
        ...
        if (enabled) {
            this.registerReportCron(time);
            this.logger.log(`Auto report generation schedule updated: ${time}`);
        } else {
            this.logger.log('Auto report generation disabled');
        }
    }

    private registerReportCron(time: string) {
        const [hours, minutes] = time.split(':');
        const cronExpression = `${minutes} ${hours} * * *`;

        const job = new CronJob(
            cronExpression,
            () => this.autoGenerateDailyReport(),
            null,
            true,
            'Asia/Dhaka',
        );

        this.schedulerRegistry.addCronJob(this.REPORT_CRON_NAME, job as any);
        job.start();
        this.logger.log(`Auto report generation cron registered: ${cronExpression}`);
    }
